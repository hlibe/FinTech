...
for random_state in range(0,1):
    X_train0, X_test0, y_train0, y_test0 = train_test_split(X, y, test_size=0.3, random_state=random_state) # data types are dataframe
    y_train1 = y_train0.values # transfer data type from dataframe to array
    y_test1 =y_test0.values 
    #X_train0 = X_train0.values
    #X_test0 = X_test0.values
    
    # oversample
    oversample_ratio = oversample_ratio # we set the ratio of the number of default=1
    overwt_in_lossfn = (1-oversample_ratio)/oversample_ratio # the value will be 4.0 if oversample_ratio = 0.2 
    recp_ratio = int(len(y_train0)/y_train0.sum()) # obtain reciprocal of the ratio of default=1
      # create a parameter of weight, if the number of default=1 is 1%, then parameter=100
    overwt_in_resampler = int(1/(1/oversample_ratio-1)*recp_ratio) # this can later help create an array of indices, which satisify oversample_ratio
    # the value will be 27, approx 1/4 of 111
    overwt_arr_resampler = []
    overwt_arr_y_lossfn = [] # create an array for weights, it will be [1,1,1,Z,1,1,...], Z may be 4, 108, ...
    for i in range(len(y_train1)): # Scan the array y_train1, if element is 0, weight = 1; if element is 1, weight = w_ytrain.
        if y_train1[i]==1: 
            overwt_arr_resampler = np.append(overwt_arr_resampler, overwt_in_resampler) 
            overwt_arr_y_lossfn = np.append(overwt_arr_y_lossfn, overwt_in_lossfn) 
        else:
            overwt_arr_resampler = np.append(overwt_arr_resampler, 1)
            overwt_arr_y_lossfn = np.append(overwt_arr_y_lossfn, 1)
    # y_train1 = y_train1*overwt_arr_ytrain # update y_train1 as weighted 
    # create a list of indices, for default = 1, sample more based on resample ratio; for default=0, sample less
    i_resample = list(WeightedRandomSampler(overwt_arr_resampler, len(y_train1), replacement = True )) 
    y_train2 = y_train1[i_resample] # create an array according to indices of i_resample 
    X_train2 = X_train0[i_resample] # 
    ...
    X_train = torch.from_numpy(X_train2) # transfer to Tensor, no need to add .double(), because it is already float data type
    y_train = torch.from_numpy(y_train2).double() # .double() makes the data type float, 在pytorch中，只有浮点类型的数才有梯度
    ...
    train = data_utils.TensorDataset(X_train, y_train, weights_tensor) # adjust format. 打包X Y 用于训练 
    ...
    for epoch in range(training_epochs):
        ...
        for b, data in enumerate(train_loader, 0): # 取batch
            inputs, labels, weights = data#.cuda() # inputs and labels follows that when loaded
            if torch.cuda.is_available():
            ...
            loss_fn = nn.BCELoss(weight = weights) # binary cross entropy loss, with weights
            if torch.cuda.is_available():
    ...
    X_test = torch.from_numpy(X_test0) # check the tested results
    y_test = torch.from_numpy(y_test1).double()
    ...
    print('resample: {}, overweight: {}, Epochs: {}, batch size: {}, '.format(oversample_ratio, overwt_in_lossfn, training_epochs, minibatch_size))
    print(net)
    '''
    test_input_tensor = torch.from_numpy(col_names).type(torch.FloatTensor)
    ...
    dropout=%s, 24=>%s=>%s=>1, random_state = %s,
    myoversampling
    '''%(training_epochs,minibatch_size,learning_rate, penalty, weight_decay, 
    dropout, layer1, layer2, random_state)
    ...
    plt.show()
    list_rec_NN.append([auc(train_fpr, train_tpr), auc(test_fpr, test_tpr),
                     penalty,
                     training_epochs,
                     minibatch_size,
                     learning_rate,
...
list_rec_1 = list_rec_NN
df = pd.DataFrame(list_rec_NN, columns = ['IS_AUC','OOS_AUC',
                                        'penalty',
                                        'training_epochs',
                                        'minibatch_size',
                                        'learning_rate',
...
df.to_csv('NN_AUC_parameter_record.csv')                      