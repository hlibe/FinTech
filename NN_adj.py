...
weight_decay=0.007 #weight_decay 就是 L2 正则项
dropout=0.25
...
for layer1 in [layer1]:
    for layer2 in [layer2]:
        for weight_decay in [0.011]:
            for training_epochs in [50]:
                for minibatch_size in [5000]:
                    for random_state in [0]:
                        ...
                        dropout=%s, 24=>%s=>%s=>%s=>1, myoversampling, random_state=%s
                        '''%(training_epochs,minibatch_size,learning_rate, 
                        penalty, weight_decay, dropout, layer1, layer2, layer3, random_state)
                        ...
                        list_rec.append([auc(train_fpr, train_tpr), auc(test_fpr, test_tpr),
                                   training_epochs,minibatch_size,learning_rate, 
                                   penalty, weight_decay, dropout, layer1, layer2, layer3,
                                   random_state
                                   ])
...
df = pd.DataFrame(list_rec, columns = ['IS_AUC','OOS_AUC','training_epochs',
                                       ...
                                       'layer1', 'layer2', 'layer3', 'random_state'])
